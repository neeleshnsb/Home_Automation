...
#     endpoint = "tekken"
#     dist = await database.child(endpoint).get_async().val()
...
async def toggle_plug1(request):
    params = {}
    await plug.update()  # Request an update
    ...
    if database.child('Coordinates(Plug1)').get().val() == None:
        print("hi")
        ...
        task = database.child("Coordinates(Plug1)").child(
            "Coordinate 1").set({"x_axis": curr_hour, "y_axis": res})
    else:
        print("ho")
        cnt = 0
        coll = database.child('Coordinates(Plug1)').get().val()
    ...
    return render(request, ['plug1.html'], {'params': params, 'data': dataJSON, 'dataHTML': dataHTML})


async def refresh_plug1(request):
    params = {}
    await plug.turn_on()
    await plug.update()  # Request an update
    ...
    return render(request, ['plug1.html'], {'data': dataJSON, 'dataHTML': dataHTML})
