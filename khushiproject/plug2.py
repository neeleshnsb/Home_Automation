...
async def toggle_plug2(request):
    params = {}
    ...
    if database.child('Coordinates(Plug2)').get().val() == None:
        print("hi")
        ...
        task = database.child("Coordinates(Plug2)").child(
            "Coordinate 1").set({"x_axis": curr_hour, "y_axis": res})
    else:
        print("ho")
        cnt = 0
        coll = database.child('Coordinates(Plug2)').get().val()
    ...
    return render(request, ['plug2.html'], {'params': params, 'data': dataJSON, 'dataHTML': dataHTML})

async def refresh_plug2(request):
    params = {}
    await plug.turn_on()
    await plug.update()  # Request an update
    ...
    return render(request, ['plug2.html'], {'data': dataJSON, 'dataHTML': dataHTML})
