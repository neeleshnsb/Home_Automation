...
async def toggle_switch2(request):
    params = {}
    ...
    if database.child('Coordinates(Switch2)').get().val() == None:
        print("hi")
        ...
        task = database.child("Coordinates(Switch2)").child(
            "Coordinate 1").set({"x_axis": curr_hour, "y_axis": res})
    else:
        print("ho")
        cnt = 0
        coll = database.child('Coordinates(Switch2)').get().val()
    ...
    return render(request, ['switch2.html'], {'params': params, 'data': dataJSON, 'dataHTML': dataHTML})

async def refresh_switch2(request):
    params = {}
    await plug.turn_on()
    await plug.update()  # Request an update
    ...
    return render(request, ['switch2.html'], {'data': dataJSON, 'dataHTML': dataHTML})
